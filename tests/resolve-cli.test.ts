...
test('resolve returns explicit empty_branch status when the gc-branch has no docs', async () => {
  ...
  try {
    ...
    result = await runCli(['resolve', '--home', home, '--query', 'auth token rotation', '--json']);
    assert.equal(result.code, 0, result.stderr);
...
test('resolve defaults to compact keyword-oriented text output', async () => {
  const home = await mkdtemp(join(tmpdir(), 'gctree-cli-resolve-text-'));
  ...
  try {
      ...
      JSON.stringify(
        {
          ...
          docs: [
            {
              title: 'Domain: Giap',
              summary: 'Giap는 quota-token 토큰 도메인이다.',
              body: 'Giap minting and quota-token workflows live here.',
              indexEntries: ['GIAP', 'quota-token'],
            },
          ],
        },
        null,
    ...
    result = await runCli(['resolve', '--home', home, '--query', 'GIAP quota-token']);
    assert.equal(result.code, 0, result.stderr);
    assert.match(result.stdout, /\[gc-tree\] 관련 문서 1개/);
    assert.match(result.stdout, /\[quota-token\] 관련 문서 존재/);
    assert.match(result.stdout, /\[quota-token\] 세부 정보 탐색: gctree show-doc --id "domain\/giap"/);
    assert.doesNotThrow(() => JSON.parse(JSON.stringify({ output: result.stdout })));
