...
test('resolve defaults to compact keyword-oriented text output', async () => {
  ...
  try {
      ...
      JSON.stringify(
        {
          ...
          docs: [
            {
              title: 'Domain: Como',
              summary: 'Como는 투표권 토큰 도메인이다.',
              body: 'Como minting and voting-token workflows live here.',
              indexEntries: ['COMO', '투표권'],
            },
          ],
        },
        null,
    ...
    result = await runCli(['resolve', '--home', home, '--query', 'COMO 투표권']);
    assert.equal(result.code, 0, result.stderr);
    assert.match(result.stdout, /\[gc-tree\] 관련 문서 1개/);
    assert.match(result.stdout, /\[COMO\] 관련 문서 존재/);
    assert.match(result.stdout, /\[COMO\] 세부 정보 탐색: gctree show-doc --id "domain\/como"/);
