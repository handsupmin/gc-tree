...
test('init persists the preferred provider, creates main gc-branch, and prepares guided onboarding', async () => {
  ...
  try {
    ...
    assert.match(parsed.launch.args[0]!, /final summary, do not ask an open-ended validation question first/i);
    assert.match(parsed.launch.args[0]!, /1\. This matches well enough\./i);
    assert.match(parsed.launch.args[0]!, /2\. Some parts are wrong\. I will give the delta\./i);
    assert.match(parsed.launch.args[0]!, /3\. The frame is wrong\. I will restate it\./i);
    assert.match(parsed.launch.args[0]!, /propose the concrete repository candidates/i);
    assert.match(parsed.launch.args[0]!, /1\. Map these candidates\./i);
    assert.match(parsed.launch.args[0]!, /2\. Map these, but with corrections\./i);
    assert.match(parsed.launch.args[0]!, /3\. Skip repo mapping for now\./i);
    ...
    assert.match(parsed.launch.args[0]!, /Index entries are the search surface/i);
    assert.match(parsed.launch.args[0]!, /repo nicknames/i);
    assert.match(parsed.launch.args[0]!, /split glossary docs when a concept is likely to be searched directly/i);
    assert.match(parsed.launch.args[0]!, /top-level `- docs\/path\.md` with indented `  - keyword`/i);
    ...
    assert.match(await readFile(join(home, 'global-codex', 'hooks.json'), 'utf8'), /UserPromptSubmit/);
...
test('scaffold writes provider-specific gc command files and gc-branch wording', async () => {
  ...
  try {
    let result = await runCli(['scaffold', '--home', home, '--host', 'codex', '--target', codexTarget]);
    assert.equal(result.code, 0, result.stderr);
    ...
    assert.match(codexOnboardSkill, /final summary, do not ask an open-ended validation question first/i);
    assert.match(codexOnboardSkill, /1\. This matches well enough\./i);
    assert.match(codexOnboardSkill, /2\. Some parts are wrong\. I will give the delta\./i);
    assert.match(codexOnboardSkill, /3\. The frame is wrong\. I will restate it\./i);
    assert.match(codexOnboardSkill, /propose the concrete repository candidates/i);
    ...
    assert.match(codexOnboardSkill, /\$gc-update-global-context \{prompt\}/i);
    assert.match(codexOnboardSkill, /\/gc-update-global-context \{prompt\}/i);
    assert.match(codexOnboardSkill, /close this session and start fresh in a new one/i);
    ...
    assert.match(codexOnboardSkill, /Index entries are the search surface/i);
    assert.match(codexOnboardSkill, /repo nicknames/i);
    assert.match(codexOnboardSkill, /split glossary docs when a concept is likely to be searched directly/i);
    assert.match(codexOnboardSkill, /top-level `- docs\/path\.md` with indented `  - keyword`/i);
    ...
    assert.match(claudeOnboardCommand, /final summary, do not ask an open-ended validation question first/i);
    assert.match(claudeOnboardCommand, /1\. This matches well enough\./i);
    assert.match(claudeOnboardCommand, /2\. Some parts are wrong\. I will give the delta\./i);
    assert.match(claudeOnboardCommand, /3\. The frame is wrong\. I will restate it\./i);
    assert.match(claudeOnboardCommand, /propose the concrete repository candidates/i);
    ...
    assert.match(claudeOnboardCommand, /\$gc-update-global-context \{prompt\}/i);
    assert.match(claudeOnboardCommand, /\/gc-update-global-context \{prompt\}/i);
    assert.match(claudeOnboardCommand, /close this session and start fresh in a new one/i);
    ...
    assert.match(claudeOnboardCommand, /Index entries are the search surface/i);
    assert.match(claudeOnboardCommand, /repo nicknames/i);
    assert.match(claudeOnboardCommand, /split glossary docs when a concept is likely to be searched directly/i);
    assert.match(claudeOnboardCommand, /top-level `- docs\/path\.md` with indented `  - keyword`/i);
