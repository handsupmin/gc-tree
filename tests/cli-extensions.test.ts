...
test('init persists the preferred provider, creates main gc-branch, and prepares guided onboarding', async () => {
  ...
  try {
    ...
    assert.match(parsed.launch.args[0]!, /~\/sources/i);
    assert.match(parsed.launch.args[0]!, /only inspect .* after the user has pointed you at them/i);
    assert.match(parsed.launch.args[0]!, /do .*not.* inspect every source file/i);
...
test('scaffold writes provider-specific gc command files and gc-branch wording', async () => {
  ...
  try {
    ...
    assert.match(codexOnboardSkill, /~\/sources/i);
    assert.match(codexOnboardSkill, /only inspect .* after the user has pointed you at them/i);
    assert.match(codexOnboardSkill, /do .*not.* inspect every source file/i);
    ...
    assert.match(claudeOnboardCommand, /~\/sources/i);
    assert.match(claudeOnboardCommand, /only inspect .* after the user has pointed you at them/i);
    assert.match(claudeOnboardCommand, /do .*not.* inspect every source file/i);
