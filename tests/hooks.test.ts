...
test('UserPromptSubmit hook resolves matching gc-tree docs into additionalContext', async () => {
  ...
  try {
    ...
    assert.match(parsed.hookSpecificOutput?.additionalContext || '', /\[Resolve Policy\]\n/);
    assert.match(parsed.hookSpecificOutput?.additionalContext || '', /Always resolve auth policy before implementation\./);
    assert.match(parsed.hookSpecificOutput?.additionalContext || '', /details: gctree show-doc --id "resolve-policy"/);
