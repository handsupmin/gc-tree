...
const categorizedOnboardingInput = {
  branchSummary: 'Giap backend lead global context.',
  docs: [
    {
      title: 'Giap',
      category: 'domain',
      indexLabel: 'giap',
      indexEntries: ['giap', 'quota-token', 'mint-token'],
      summary: 'quota-token 토큰 도메인 개념.',
      body: 'Giap는 Abstract 블록체인에서 발행되는 quota-token 토큰이다.',
    },
    {
      title: 'Orbit',
      category: 'domain',
      indexLabel: 'orbit',
      indexEntries: ['orbit', 'event cycle'],
      summary: 'event cycle 도메인 개념.',
      body: 'Orbit는 어드민에 등록되고 온체인에 반영되는 event cycle다.',
    },
    {
      title: 'G3 Repository',
      category: 'repos',
...
const pathishOnboardingInput = {
  ...
  docs: [
    ...
    {
      title: 'Giap',
      indexLabel: 'docs/domain/giap.md',
      indexEntries: ['docs/domain/giap.md', 'giap'],
      summary: 'Giap domain concept.',
...
const prefixedOnboardingInput = {
  ...
  docs: [
    ...
    {
      title: 'Workflow: api-v3 feature development',
    ...
    {
      title: 'Domain: Giap backend glossary',
      summary: 'Glossary.',
      body: 'Giap, Orbit, Widget.',
    },
    {
      title: 'Role: Giap backend developer workflow',
...
test('onboard writes summary-first docs and a compact index', async () => {
  ...
  try {
    ...
    assert.match(index, /- docs\/project-identity.md\n  - Project Identity/);
...
test('onboard can write categorized docs and render a sectioned dictionary index', async () => {
  ...
  try {
    ...
    assert.match(index, /- docs\/domain\/giap\.md\n  - giap\n  - mint-token\n  - quota-token/);
    assert.match(index, /- docs\/domain\/orbit\.md\n  - event cycle\n  - orbit/);
    assert.match(index, /## Repos/);
    assert.match(index, /- docs\/repos\/api-v3\.md\n  - api-v3/);
    assert.match(index, /## Workflows/);
    assert.match(index, /- docs\/workflows\/db-migration\.md\n  - db-migration/);

    const giapDoc = await readFile(join(branchDocsDir(home, DEFAULT_BRANCH), 'domain', 'giap.md'), 'utf8');
    assert.match(giapDoc, /## Summary/);
    assert.match(giapDoc, /quota-token 토큰 도메인 개념/);
  } finally {
    await rm(home, { recursive: true, force: true });
  }
});
...
test('onboard normalizes path-like index labels into categorized docs and clean index entries', async () => {
  ...
  try {
    ...
    const index = await readFile(branchIndexPath(home, DEFAULT_BRANCH), 'utf8');
    assert.match(index, /## Repos/);
    assert.match(index, /- docs\/repos\/airflow\.md\n  - airflow/);
    assert.match(index, /## Domain/);
    assert.match(index, /- docs\/domain\/giap\.md\n  - giap/);
    assert.doesNotMatch(index, /docs\/docs-repos-airflow-md\.md/);
    assert.equal((index.match(/- docs\/domain\/giap\.md\n  - giap/g) || []).length, 1);
...
test('onboard infers categories from prefixed titles and skips docs/index.md', async () => {
  ...
  try {
    ...
    assert.match(index, /- docs\/repos\/airflow\.md\n  - airflow/);
    assert.match(index, /## Workflows/);
    assert.match(index, /- docs\/workflows\/api-v3-feature-development\.md\n  - api-v3 feature development/);
    assert.match(index, /## Conventions/);
    assert.match(index, /- docs\/conventions\/backend-coding-and-dto-rules\.md\n  - backend coding and DTO rules/);
    assert.match(index, /## Domain/);
    assert.match(index, /- docs\/domain\/giap-backend-glossary\.md\n  - Giap backend glossary/);
    assert.match(index, /## Role/);
    assert.match(index, /- docs\/role\/giap-backend-developer-workflow\.md\n  - Giap backend developer workflow/);
...
test('resolve deduplicates docs even when index exposes multiple entries for the same file', async () => {
  ...
  try {
    ...
    const result = await resolveContext({ home, branch: 'main', query: 'quota-token' });
    const giapMatches = result.matches.filter((match) => match.id === 'domain/giap');
    assert.equal(giapMatches.length, 1);
  } finally {
    await rm(home, { recursive: true, force: true });
  }
});
