...
- **Provider 駆動のオンボーディング**  
  `gctree init` は Codex と Claude Code のどちらを使うかを聞き、その選択を保存し、現在の環境に必要なコマンド面を用意したうえで、デフォルトの `main` gc-branch のオンボーディングを開始します。
- **リポジトリ範囲付き gc-branch**  
...
- provider（`codex` または `claude-code`）を選ばせる
- その provider を `~/.gctree/settings.json` に保存する
- 現在の環境に必要なコマンド面を用意する
...
これらのコマンドは、長期コンテキストを集めたり更新したりする前に、現在の gc-branch を必ず明示するべきです。
